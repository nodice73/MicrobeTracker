...
const double delta2 = 2E-8;
const double delta3 = 1E-7;
...
void makecw(double *x1, double *y1, int m)
{
    double area = 0;
    int i,j;
    for(i=0;i<m;i++)
...
double getArea(double *x0, double *y0, int m, double *x2c, double *y2c)
{
    ...
    double x2[4]={0,0,1,1};
    double y2[4]={0,1,1,0};
    for(i=0;i<4;i++){x2[i]=*x2c+x2[i]-0.5+0.0000123;} // introducing a small shift to avoid overlapping nodes
    for(i=0;i<4;i++){y2[i]=*y2c+y2[i]-0.5+0.0000104;}
