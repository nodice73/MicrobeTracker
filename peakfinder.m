...
if length(varargin)>=5 && length(varargin{5})==1
    ...
    smooth = 10; % degree of smoothing, cycles
end
        ...
        if ~isempty(cellList{frame}{cell})
            ...
            if f
                ...
                for i=1:length(f)
                 ...
                 %%%remove f = f+1 by Hoong 20120905;
                ...
                if ~isempty(fa)
                    ...
                    % this doesn't allow you to have multiple spots
                    % when the orientation of lengthvector was inverted
                    i = 0;
                    j = 0;
                    while i<=length(c)
                        i = i+1;
                ...
                f2 = unique(f2);
                f3=[]; cs=cumsum(cellList{frame}{cell}.steplength); for i=1:length(l3), f3=[f3 1+sum(l3(i)>cs)]; end
                str = [];
